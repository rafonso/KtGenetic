...
class LogProcessorListener : ProcessorListener {
    ...
    private fun populationToConsole(population: List<Chromosome<*>>): String {
        ...
        val genotypesByLine =
                if (population[0].valueToString().length < CONSOLE_SIZE) CONSOLE_SIZE / (population[0].valueToString().length + 1)
                else 1
    ...
    private fun debugGenerationEvaluated(event: ProcessorEvent<*>): () -> Unit {
        return {
            ...
            log.debug("Gen %3d - AF %.3f (%.3f). Best Opt: %s".
                    format(event.generation, averageFitness, averageFitnessDeviation, bestOption))
        }
    }
